    ...
    for key, value in upper.items():
        ...
        if len(cnts) > 0:
            ...
            if radius > 0.5:
                ...
                rad=str(radius)
                dist=str((0.003163*radius*radius)-(0.6808*radius)+44.38)
                cv2.circle(frame, (int(x), int(y)), int(radius), colors[key], 2)
                cv2.putText(frame,key+dist, (int(x-radius),int(y-radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.6,colors[key],2)
                #print(key,colors[key])
                print(key,center,radius)
